...
### 글 G. 팀 계정에서 개인 계정으로 — Terraform 재구축과 게이트 연결 (09.16)

| 핵심 장면 | 소스 | 활용 |
| --- | --- | --- |
| 자원 전부 삭제된 상태에서 코드로 재현 | `pf/infra/*.tf`, `infra/README.md`, 커밋 f4ff1fa~ | 구성 |
| c7i-flex.large 가 2a 미제공, 기본 서브넷이 IGW 경로 없는 라우트 테이블에 연결돼 인스턴스 완전 고립 | `infra/network.tf` 주석, apply1~3.log(로컬), 메모리 | 실패 사례 2건 |
| OIDC 역할·SSM 배포로 키 없는 파이프라인 재연결 | `infra/iam.tf`, backend 워크플로 b9cdd2f | 배포 |
| 대표 사진 출처(코리안넷 공개 조회) 복원 | `backend/demo/fetch-master-images.py` 0805b88 | 데이터 복원 |
| 게이트 잡: 이미지에서 model.onnx 추출, S3 고정셋, 기준선 부트스트랩, live 승격 | ai 워크플로 59f22b9·7b60d42·11a7b31, run 35075511854 | 파이프라인 |
| 별칭 이동 직후 첫 촬영 MEASURE_FAILED(콜드 10초 > 8초) → 배포 잡 워밍 | 백엔드 로그 08:48, CloudWatch Init Duration 9999ms | 실패 사례 |
| 측정용 복제 사진이 추론 입력으로 쓰여 결과 왜곡(스팸 49×48×20) | S3 images/ md5 동일, `tools/eval/drive_to_s3.sh` | 실패 사례 |
| 운영 실측 55회 | `docs/evidence/measurement/prod-2026-09-16.md` | 결과 |
...
1. 글 순서 결정. 후보: B(카토나이제이션) → C(서빙) → D(1초) → E(게이트) → G(재구축) → A(팀 운영). F는 보류.
